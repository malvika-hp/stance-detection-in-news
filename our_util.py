...
def pack_labels_basic_lstm_senti(data, labels, seqlen, sentiment_data=[], sentiment_seqlen=[]): # MODIF
    output = []
    num_rows = data.shape[0]
    assert num_rows == len(labels)
    if sentiment_data.size!=0:
        for i in range(data.shape[0]):
            the_row = data[i, :]
            output.append((the_row, labels[i], seqlen[i], sentiment_data[i], sentiment_seqlen[i]))
    else:
        for i in range(data.shape[0]):
            the_row = data[i, :]
            output.append((the_row, labels[i], seqlen[i]))        
...
def get_minibatches_lstm_sentiment(data, minibatch_size, shuffle=True, sentiment = False):
    ...
    for minibatch_start in np.arange(0, data_size, minibatch_size):
        ...
        if list_data:
            examples_minibatch = minibatch_lstm_sentiment(data[0], minibatch_indices) # np.array of shape (batch_size, max_length_global)
            labels_minibatch = minibatch_lstm_sentiment(data[1], minibatch_indices)
            seqlen_minibatch = minibatch_lstm_sentiment(data[2], minibatch_indices)
            # for sentiments
            if( sentiment ):
                sentiment_minibatch = minibatch_lstm_sentiment(data[3], minibatch_indices)
                sentiment_seqlen_minibatch = minibatch_lstm_sentiment(data[4], minibatch_indices)
                yield [examples_minibatch, labels_minibatch, seqlen_minibatch, sentiment_minibatch, sentiment_seqlen_minibatch]
        ...
        # Not sure where is this being called,have not yet handled sentiment here
        else: # no truncating if data not in the 'packed' list format [examples, labels, seqlen]
            yield minibatch_lstm_sentiment(data, minibatch_indices)


## Derived from Stanford CS 224n started code provided for assignment 3.
def minibatch_lstm_sentiment(data, minibatch_idx):
    return data[minibatch_idx] if type(data) is np.ndarray else [data[i] for i in minibatch_idx]

def minibatches_lstm_sentiment(data, batch_size, sentiment=False, shuffle=True):
    batches = [np.array(col) for col in zip(*data)]
    return get_minibatches_lstm_sentiment(batches, batch_size, shuffle, sentiment)
