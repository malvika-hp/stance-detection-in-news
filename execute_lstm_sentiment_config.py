...
######
# Initial test of LSTM model for Fake News Challenge - Using actual data
...
from run_text_processing import get_data, save_data_pickle, get_lexicon_data

from our_util import Progbar, minibatches, pack_labels, split_data, softmax, get_performance, convertOutputs, downsample_label, pack_labels_basic_lstm_senti

from basicLSTM_sentiment_model_config import *
...
def run_lstm(config, outputpath = '../../xp', final = False):
    ...
    config1, data_dict_l = get_lexicon_data(config,
                             filename_embeddings = '/../../glove/glove.twitter.27B.50d.txt',
    ...
    sentiment_h_b_np = data_dict_l['h_b_np']
    sentiment_seqlen = data_dict_l['seqlen']
    ...
    if config1.max_length is not None:
        max_length = config.max_length
        if np.shape(sentiment_h_b_np)[1] > max_length:
            sentiment_h_b_np = sentiment_h_b_np[:, 0:max_length]
        sentiment_seqlen = np.minimum(sentiment_seqlen, max_length)   

    # Set maximum dataset size for testing purposes
    data = pack_labels_basic_lstm_senti(h_b_np, y, seqlen, sentiment_h_b_np, sentiment_seqlen)
    ...
    dev_sentiment_data_np  = sentiment_h_b_np[dev_indices, :]
    dev_sentiment_seqlen = sentiment_seqlen[dev_indices]

    # Test
    test_labels = y[test_indices]
    test_data_np = h_b_np[test_indices, :]
    test_seqlen = seqlen[test_indices]
    test_sentiment_data_np = sentiment_h_b_np[test_indices, :]
    test_sentiment_seqlen = sentiment_seqlen[test_indices]
    ...
    if final:
        ...
        dev_sentiment_data_np = test_sentiment_data_np
        dev_sentiment_seqlen = test_sentiment_seqlen
    ...
    with tf.Graph().as_default():
        ...
        with tf.Session() as session:
            session.run(init)
            losses_ep, dev_performances_ep, dev_predicted_classes_ep, dev_predictions_ep = model.fit(session, train_data, dev_data_np, dev_seqlen, dev_labels, dev_sentiment_data_np, dev_sentiment_seqlen) # MODIF
