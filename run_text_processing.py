...
def lconcatConvert_np(h_list, b_list):
    # Concatenate
    n_sentences = len(b_list)
    h_b_list = []
    seqlen = []
    for i in range(n_sentences):
        #h_b_list.append(h_list[i] + b_list[i])
        h_b_list.append(b_list[i])
        seqlen.append(len(h_b_list[i]))
...
def get_lexicon_data(config, 
    ...
    b_ids = data_dict['b_ids']
    y = data_dict['y']
    print('finished loading lexicon Pickle')
    max_len = 75

    for i in range(len(b_ids)):
        if len(b_ids[i]) > max_len:
            b_ids[i] = b_ids[i][:75]
    
    b_ids, b_np, seqlen = lconcatConvert_np(h_ids, b_ids)
    output_dict = {'y':y,
                   'h_b_np':b_np, 
                   'seqlen':seqlen}
...
def lwords2ids_vects(sentences, voc_dict, embedding_matrix, d):
    ...
    for sentence in sentences:
        ...
        for word in word_list:
            if word.lower() in voc_dict and word.lower() in d: # Only add word if in dictionary
                word_index = voc_dict[word.lower()]
                sentence_ids.append(word_index)
                sentence_vects.append(embedding_matrix[word_index])
...
def save_lexicon_data():
    ...
    # GloVe embeddings
    vocab, embedding_matrix = loadGloVe(filename_embeddings)
    vocab_size = len(vocab)
    ...
    # Read and process data
    h, b, y = read_data(cwd + '/../../') # headline / bodies/ labels
    # h_ids, h_vects = lwords2ids_vects(h, voc_dict, embedding_matrix, d)
    b_ids, b_vects = lwords2ids_vects(b, voc_dict, embedding_matrix, d)

    data_dict = {'b_ids':b_ids, 'b_vects': b_vects, 'y':y}
    with open('lexiconp.p', 'wb') as fp:
        pickle.dump(data_dict, fp)
        print("Saved pickle")

def lconcatConvert_np_bilstm_sentiment(b_list):
...
def get_lexicon_data_bilstm_sentiment(config, 
            filename_embeddings = '/../glove/glove.twitter.27B.50d.txt',
            pickle_path = 'lexiconp.p',
            concat = True):
    cwd = os.getcwd()
    ...
    y = data_dict['y']
    print('finished loading lexicon Pickle')
    max_len = config.b_max_len

    for i in range(len(b_ids)):
        if len(b_ids[i]) > max_len:
            b_ids[i] = b_ids[i][: config.b_max_len]
    
    b_ids, b_np, seqlen = lconcatConvert_np_bilstm_sentiment(b_ids)
    output_dict = {'y':y,
                   'h_b_np':b_np, 
                   'seqlen':seqlen}
...
#     # ========== YOUR OWN EMBEDDING MATRIX PATH HERE =========
    filename_embeddings = os.getcwd() + '/glove/glove.6B.50d.txt'
